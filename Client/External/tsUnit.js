...
class Test {
    constructor(testModules) {
        ...
        for (var i = 0; i < testModules.length; i++) {
            ...
            for (var testClass in testModule.prototype) {
                if (testClass && /Should/.test(testClass)) {
                    this.addTestClass(new testModule(testModule.prototype[testClass]), testClass);
                }
            }
        }
    }
    ...
    run(testRunLimiter = null) {
        ...
        for (var i = 0; i < this.tests.length; ++i) {
            ...
            var unitTestName = testsGroupName;
